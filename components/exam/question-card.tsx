    ...
    <div className="bg-white rounded-xl md:rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border-b border-gray-200 p-4 md:p-6">
        <div className="flex items-start justify-between gap-3 md:gap-4 mb-3 md:mb-4">
          <h2 className="text-base md:text-xl font-bold text-gray-900 flex-1">{question.content}</h2>
          <div className="flex items-center gap-2 md:gap-3 flex-shrink-0">
            ...
            <a
              ...
              className="flex items-center gap-1 md:gap-2 px-2 md:px-3 py-1.5 md:py-2 bg-green-500 hover:bg-green-600 text-white rounded-lg transition-colors font-semibold text-xs md:text-sm"
        ...
        {/* Imagen si existe - responsiva */}
        {question.image_url && (
          <div className="mt-3 md:mt-4 rounded-lg overflow-hidden bg-gray-100">
            <img
      ...
      <div className="p-4 md:p-6 space-y-2 md:space-y-3">
        <p className="text-xs md:text-sm font-semibold text-gray-700 mb-3 md:mb-4">Elige una respuesta:</p>
        ...
        {optionsWithLetters.map((option) => (
          ...
          >
            <div className="flex items-center gap-3 md:gap-4">
              {/* Indicador de opción */}
              <div
                className={`flex-shrink-0 w-8 md:w-10 h-8 md:h-10 rounded-full border-2 flex items-center justify-center font-bold text-xs md:text-base ${
                  selectedAnswer === option.value
              ...
              {/* Texto de opción */}
              <span className="text-sm md:text-base text-gray-900 font-medium">{option.value}</span>
            </div>
          </button>
        ))}
      </div>

      {/* Explicación (toggle) - adaptada a móvil */}
      <div className="border-t border-gray-200 p-4 md:p-6 bg-gray-50">
        <button
          onClick={() => setShowExplanation(!showExplanation)}
          className="flex items-center gap-2 text-xs md:text-sm font-semibold text-blue-600 hover:text-blue-700"
        >
        ...
        {showExplanation && (
          <div className="mt-3 p-3 md:p-4 bg-blue-50 border border-blue-200 rounded-lg text-xs md:text-sm text-gray-800">
